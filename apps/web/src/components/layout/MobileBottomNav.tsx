...
export function MobileBottomNav({ itemCount }: MobileBottomNavProps) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-4 z-[100] flex justify-center px-4 md:hidden">
      <nav className="type-nav relative flex w-full max-w-sm items-center justify-between rounded-full border border-white/80 bg-white/45 px-2 py-2 backdrop-blur-2xl shadow-[0_18px_40px_rgba(15,23,42,0.16)]">
        ...
        {mobileNavLinks.map((link) => (
          <Link
            ...
            href={link.href}
            className={cn(
              "relative z-10 flex h-11 min-w-[52px] flex-col items-center justify-center gap-0.5 rounded-full px-1.5 transition-all",
              link.active
