-- Minimal auth schema for Postgres-only VPS (admin login via promote-admin).
-- Run as DB superuser, e.g.:
--   sudo -u postgres psql -d icrowd -f scripts/bootstrap-local-auth.sql
