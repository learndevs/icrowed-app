...
if [[ -z "${SUPABASE_DATABASE_URL}" && -f "${ENV_FILE}" ]]; then
  SUPABASE_DATABASE_URL="$(load_env_var DIRECT_URL)"
  if [[ -z "${SUPABASE_DATABASE_URL}" ]]; then
    src="$(load_env_var DATABASE_URL)"
...
if [[ -z "${LOCAL_DATABASE_URL}" && -f "${ENV_FILE}" ]]; then
  LOCAL_DATABASE_URL="$(load_env_var DATABASE_URL)"
  if [[ "${LOCAL_DATABASE_URL}" == *"supabase.com"* ]]; then
    echo "LOCAL_DATABASE_URL not set and .env.local still points to Supabase."
    echo "Run vps-postgres-setup.sh first, then set LOCAL_DATABASE_URL."
    exit 1
  fi
fi
